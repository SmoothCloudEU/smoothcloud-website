(function(){
  ...
  function createBubble(sideContainer, side) {
    ...
    // Größe
    const minSize = parseFloat(getComputedStyle(document.documentElement).getPropertyValue('--bubble-min-size')) || 14;
    const maxSize = parseFloat(getComputedStyle(document.documentElement).getPropertyValue('--bubble-max-size')) || 110;
    const size = rand(minSize, maxSize);

    // Farbe / Tönung
    const hueBase = parseFloat(getComputedStyle(document.documentElement).getPropertyValue('--bubble-hue-base')) || 218;
    const hue = hueBase + rand(-6, 10);
    const lightMin = parseFloat(getComputedStyle(document.documentElement).getPropertyValue('--bubble-light-min')) || 18;
    const lightMax = parseFloat(getComputedStyle(document.documentElement).getPropertyValue('--bubble-light-max')) || 38;
    const light = rand(lightMin, lightMax);
    const sat = getComputedStyle(document.documentElement).getPropertyValue('--bubble-sat') || '65%';
    const alpha = parseFloat(getComputedStyle(document.documentElement).getPropertyValue('--bubble-alpha')) || 0.28;

    bubble.style.width = size + 'px';
    bubble.style.height = size + 'px';
    bubble.style.left = side === 'left'
      ? rand(-15, 55) + '%'
      : rand(45, 115) + '%';

    // vertikale Startposition (leichte Verschiebung)
    bubble.style.bottom = rand(-5, 25) + '%';

    const scale = rand(0.85, 1.25);
    const duration = rand(18, 42);            // Aufstieg
    const delay = rand(-duration * 0.9, 4);   // negative Verzögerung für zeitlich versetzte Loops
    ...
    const wobbleX = side === 'left'
      ? rand(4, 18) * (Math.random() > 0.5 ? 1 : -1)
      : rand(4, 18) * (Math.random() > 0.5 ? 1 : -1);

    // Ziel-Opazität abhängig von Größe (kleinere = weniger sichtbar)
    ...
    // Wobble Parameter (zufällige Keyframe Offsets)
    ...
    // Innenlayer für Wobble (trennt Aufstiegs- und Wobble-Animation)
    ...
    bubble.addEventListener('animationend', (ev) => {
      if (ev.animationName === 'bubble-rise') {
        // Reset Parameter, so dass kein neues Element erzeugt werden muss
        ...
        bubble.style.animation = `
          bubble-rise linear forwards,
          bubble-fade-in 1400ms ease forwards
        `;
        bubble.style.animationDuration = `${nDur}s, 1400ms`;
        bubble.style.animationDelay = `${nDelay}s, ${nDelay}s`;
        requestAnimationFrame(()=> {
          // Force reflow not nötig – CSS Re-Anwendung ausreichend
        });
      }
    }, { passive: true });
  }

  // Initiale Erzeugung (hälfte links / rechts, leichte Variation)
